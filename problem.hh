...
public:
    ...
    PrimaryVariables initialAtPos(const GlobalPosition& globalPos) const
    {
        ...
        if(globalPos[0] > 5.0 - eps_ && globalPos[0] < 19.0 + eps_)
            ...
            priVars[precipNaClIdx] = 0.0; // 0.0 [kg/m^3]
                   ...
                   const SubControlVolume &scv) const
    {
        ...
        /*  //something like KIS reaction
        Scalar precipSalt2 = volVars.porosity() * volVars.molarDensity(liquidPhaseIdx)
                                                       * volVars.saturation(liquidPhaseIdx)
                                                       * 0.01
                                                       / timeStepSize_;
        if ((volVars.saturation(liquidPhaseIdx) < 0.41)) // || (volVars.saturation(liquidPhaseIdx) > 0.75))
        	precipSalt2 = 0;
        */

        source[conti0EqIdx + NaClIdx] += -precipSalt; //-precipSalt;
        source[precipNaClEqIdx] += 0.0;  //+= precipSalt;  cancel precipitation
        return source;
